...
describe("performSteps pickByTypeofTests test", () => {
    beforeEach(async () => {
        jest.clearAllMocks();
        getDefaultArgsMock.mockImplementation(() => ({
            ...getDefaultPerformStepsIntegrationTestsArgsBaseTests(),
            srcFolder: "./testdataWithAdditionalTests/pickByTypeofTests",
            templatesPath: "./testdataWithAdditionalTests/pickByTypeofTests/templates",
        }));
    });

    it("Given performSteps exists When called for pickByTypeofTests Then it performs", async () => {
