...
import { isInFormMappingExpression } from "../isInFormMappingExpression";

/**
 * returns an @see {InFormMappingExpression} if node is inheriting InFormMapping interface otherwise null
 * @param node
 * @param doc
 * @returns
 */
...
): Promise<InFormMappingExpression | null> {
        ...
        for await (const type of child.types) {
            switch (type.kind) {
                case ts.SyntaxKind.ExpressionWithTypeArguments:
                    if (isInFormMappingExpression(type, sourceFile)) {
                        return type;
